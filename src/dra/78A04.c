...
void EntityNumberMovesToHpMeter(Entity* self) {
    ...
    case 5:
        if (--self->ext.hpNumMove.unk90 == 0) {
            ...
            self->ext.hpNumMove.angleToMeter = ratan2(-y_to_meter, x_to_meter) & 0xFFF;
            self->ext.hpNumMove.distToMeter = SquareRoot0(SQ(x_to_meter) + SQ(y_to_meter));
    ...
    case 6:
        ...
        self->posX.i.hi = 13 + (((rcos( (s16) temp_s0) >> 4) * (s16) temp_s1_2) >> 8);
        self->posY.i.hi = 70 - (((rsin( (s16) temp_s0) >> 4) * (s16) temp_s1_2) >> 8);
...
INCLUDE_ASM("dra/nonmatchings/78A04", EntityGuardText);
